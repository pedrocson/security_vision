...
def deletar_placa(placa):
    ...
    if doc.exists:
        ...
        confirma= f'Erro - A placa {valor} não existe no banco!'
        return confirma
...
def buscar_historico(placa):
    plates_report = db.collection(u'historico_ref').document(placa).get()
    plates = []
    
    if plates_report.exists:
        histref = hist_ref(placa)  # Obtenha o histórico para a placa específica
        plates.append(f"Placa: {placa} - {histref}")
...
def buscar():
    placa = request.form.get('placa')
    try:
        historico = buscar_historico(placa)
        return render_template('relatorio_busca.html', rel=historico)
    except Exception as e:
        historico = "Ocorreu um erro ao buscar o histórico."
        return render_template('relatorio_busca.html',rel=historico)
